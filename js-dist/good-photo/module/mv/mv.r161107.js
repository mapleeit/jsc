...
define.pack("./image_lazy_loader",["$","lib","common","./user_log","./app_cfg","./store"],function(require, exports, module) {
    ...
    var lazy_loader = new Module('lazy_loader', {
        ...
        //加载单个页面
        lazy_load: function(item) {
            var me = this;
            window.requestAnimationFrame(function() {
                me._lazy_load_image(item);
            });
        },

        _lazy_load_image: function(item) {
            var divs = item.find('[data-src]');

            divs.each(function(i, dom) {
                var $dom = $(dom);
                var is_big = $dom.attr('data-size') === 'big';
                if(!$dom.attr('data-loaded')) {
                    var thumb_url = $dom.attr('data-src');
                    if(!is_big) {
                        thumb_url = thumb_url.replace(/\/640\?/,'/256?');
                    }
 ...
 * @date 2016-12-30
 */
define.pack("./like",["$","lib","common","./user_info","./user_log","./app_cfg","./retry","./store","./tmpl"],function(require, exports, module) {
    var $ = require('$'),
        lib = require('lib'),
        common = require('common'),
    ...
    var like = new Module('like', {
        ...
        insert_$tips: function() {
            if(this.get_$like_info() && this.get_$like_info().length) {
                $('.j-like-info').remove();
                this.$like_info = null;
            }
            var is_join = store.is_join_album();
            $(tmpl.like_tips({
                is_join: is_join
            })).appendTo(this.get_$ct());
        },
        ...
        add_like: function() {
            ...
            this.send_request(true).done(function(body) {
                ...
                if(ret_msgs.is_sess_timeout(ret) && browser.WEIXIN) {
                    retry.init({
                        name: 'auto_like'
                    });
                }
    ...
    return like;
});define.pack("./mgr",["lib","$","common","./user_info","./user_log","./app_cfg","./store","./retry","./like"],function(require, exports, module) {
    var lib = require('lib'),
        $ = require('$'),
        common = require('common'),
    ...
    var mgr = new Mgr('mv.mgr', {
        ...
        on_save: function(data) {
            if(this._requesting) {
                widgets.reminder.error('on_save');
                return;
            }
        ...
        on_confirm_join: function() {
            var me = this,
                is_join = store.is_join_album();

            if(app_cfg.is_hzp_app() || is_join) {
                this.do_enter_group();
                return;
            }
        ...
        on_enter_group: function() {
            var me = this,
                dir_key = store.get_dir_key(),
                invite_nickname = store.get_nickname(),
                _user_info = user_info.get_user_info(),
                is_join = store.is_join_album();

            if(app_cfg.is_hzp_app() || is_join) {
                this.do_enter_group();
                return;
            } else if(!user_info.is_login()) {
                if(browser.WEIXIN) {
                    retry.init({
                        name: 'auto_join'
                    });
                    ...
                    //非微信场景则仍然去加入，但需要上报错误码。后台会针对这种没有头像昵称的会自己拉取。
                }
            }
            if(this._requesting) {
                return;
        ...
        do_enter_group: function() {
            ...
            schema_url = schema_url + '/' + dir_key + ((!app_cfg.is_hzp_app() && user_id)? '?user_id=' + user_id : '');
    ...
    var mv = new Module('mv', {
        ...
        render: function(serv_rsp) {
            ...
            var share_url = location.href + (location.search.indexOf('_wv=2098177')>-1? '' : '&_ws=5&_wv=2098177'),
                share_data = {
...
define.pack("./retry",["$","lib","common","./user_log"],function(require, exports, module) {
    ...
    var action_list = ['auto_like', 'auto_join'];
 ...
 * @date 2014-12-23
 */
define.pack("./ui",["lib","$","common","./image_lazy_loader","./input_box","./store","./mgr","./like","./music","./tmpl","./retry","./address","./user_log","./app_cfg"],function(require, exports, module) {
    var lib = require('lib'),
        $ = require('$'),
        common = require('common'),
        widgets = common.get('./ui.widgets'),
    ...
    var ui = new Module('ui', {
        ...
        render: function() {
            if(store.is_preview()) {
                ...
                //处理缩略图
                image_lazy_loader.lazy_load($now);
        ...
        listen_auto_events: function() {
            var me = this;
            this.listenTo(retry, 'auto_like', function() {
                me.slide_play(false, 1);
                mgr.on_like();
            }).listenTo(retry, 'auto_join', function() {
                me.slide_play(false, 1);
                mgr.on_enter_group();
            });
        },
        ...
        slide_play: function(slideTop, index) {
            ...
            image_lazy_loader.lazy_load($next);
...
__p.push('</span>\r\n\
            ...
            <!-- 底部 S -->\r\n\
            <div class="photo-ft">');
 if(dir_name) { __p.push('                <div class="from"><p>摘自<span data-action="open_app">《');
_p(dir_name);
__p.push('》</span></p></div>');
