...
define("club/weiyun/js-dist/appbox-i18n-multi/module/download_route/download_route.r1104",["common","main","lib","upload"],function(require,exports,module){
 ...
 * @date 13-3-27
 */
define.pack("./download_route",["common","main","lib","upload"],function (require, exports, module) {
        ...
        deal_percent_cache = functional.throttle(function () {//每300毫秒更新一次进度cache
            for (var key in percent_cache) {
                ...
                if (task) {
                    ...
                    if(task.state === 'start' || task.state === 'continuee' || task.state === 'processing') {
     ...
     * @constructor
     */
    window.WYCLIENT_AddDownloadTask = function (task_id, file_name, file_size, target_path, file_thum_url, cover_file, OperationType) {
        if (!Class) {
            var mod = require('upload');
            Cache = mod.get('./tool.upload_cache');
            Class = mod.get('./download.appbox');
        }
        console.debug('WYCLIENT_AddDownloadTask', task_id, file_name, file_size, OperationType);
        Class.create({
            'task_id': task_id,
            'file_name': file_name,
            'file_size': file_size,
    ...
    window.WYCLIENT_OnDownloadTaskCompleted = function (task_id, target_path, error_code, file_name, file_size) {
        ...
        delete percent_cache[task_id];
        console.debug('OnDownloadTaskCompleted', task_id, error_code, (error_code == 0 || error_code == 1), file_name, task);
