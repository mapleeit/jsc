...
define("club/weiyun/js-dist/appbox-i18n-multi/module/recent/recent.r112901",["lib","common","$","i18n","main"],function(require,exports,module){
 ...
 * Time: 下午7:03
 */
define.pack("./file_list.file",["lib","common","$","i18n"],function (require, exports, module) {
    var lib = require('lib'),
    ...
    return {
            ...
            $.each(list, function (i, unit) {
                if (unit) {//unit可能为空
                    ...
                    var file = new File_Node({
                        ...
                        date = date_time.parse_str(file.get_create_time()),
                        time = date.getTime();
                    //按日期分类
                    if (time >= today) {
        ...
        _get_html_time: function(date , type){
            var me = this;
            if(type==='T'){
                return _(l_key,'今天')+' ' + me.fixNum(date.getHours()) + ':' + me.fixNum(date.getMinutes());
            } else if( type === 'Y'){
                return _(l_key,'昨天')+' ' + me.fixNum(date.getHours()) + ':' + me.fixNum(date.getMinutes());
            } else {
 ...
 * Time: 下午7:03
 */
define.pack("./file_list.file_list",["lib","common","$","main","./ui","./file_list.file","./file_list.ui"],function (require, exports, module) {
    var lib = require('lib'),
        common = require('common'),
        $ = require('$'),
    ...
    var ui = new Module('recent_ui', {
        ...
        _get_paint_unit: function (ary, type) {
            ...
            if (ary && ary.length) {
                ...
                if (html && ary[0].get_id() === file_data[type][0].get_id()) {//首次渲染把单元标题加上
                    return tmpl['bar']({'time': type}) + html;
                }
            }
            return html;
        },
        ...
        paint: function (is_first_page) {
            ...
            if (last7day_queue.length > 0 && paint_count > 0) {
                html.push(me._get_paint_unit(last7day_queue.splice(0, paint_count), 'S'));
            }
 ...
 * Time: 下午7:03
 */
define.pack("./ui",["common","$","main","./tmpl"],function (require, exports, module) {
...
__p.push('    ');
        ...
        var _ = require('i18n').get('./pack'),
            l_key = 'recent';
    __p.push('    <div id="_recent_files_empty" class="g-empty sort-cloud-empty">\r\n\
        <div class="empty-box">\r\n\
            <div class="ico"></div>\r\n\
            <p class="title">');
_p(_(l_key,'暂无文件'));
...
__p.push('    ');
    ...
    $.each(files, function (i, file) {
        var is_image = file.is_image(); __p.push('        <div data-file-id="');
_p(file.get_id());
__p.push('"');
...
_p(default_image);
__p.push('" unselectable="on" style="border-width:0px;"/>');
} else {__p.push('                <!-- 图标 -->\r\n\
...
__p.push('    ');
        ...
        var click_tj = require('common').get('./configs.click_tj'),
            _ = require('i18n').get('./pack'),
            l_key = 'recent';
    __p.push('    <a href="javascript:void(0)" class="load-more"><i class="icon-loading"></i>');
_p(_(l_key,'正在加载'));
__p.push('</a>');
...
__p.push('    ');
      ...
      var click_tj = require('common').get('./configs.click_tj'),
          _ = require('i18n').get('./pack'),
          l_key = 'recent';
    __p.push('    <a href="javascript:void(0)" class="load-more" style="cursor: default;">');
_p(_(l_key,'加载完成'));
__p.push('</a>');
...
var __p=[],_p=function(s){__p.push(s)};
__p.push('    <div id="_recent_body" class="disk-view ui-view ui-listview">\r\n\
...
_p(( data.time==='T'?_(l_key,'今天'):( data.time==='Y'?_(l_key,'昨天'):( data.time ==='S'?_(l_key,'最近7天'):_(l_key,'7天前') ) ) ));
__p.push('</h3>');
