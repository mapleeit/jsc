...
define("club/weiyun/js-dist/appbox-i18n-multi/module/compress_file_preview/compress_file_preview.r0102",["lib","common","$","webbase_css","disk_css","downloader","i18n"],function(require,exports,module){
...
define.pack("./request",["lib","common","$"],function (require, exports, module) {
    ...
    var list = function (file, file_path, fail_callback, succ_callback) {
        ...
        last_req = request.post({
            url: 'http://web.cgi.weiyun.com/weiyun_web_zip.fcg',
            cmd: 'list_zip',
            body: param
        })
            .fail(function (msg, ret, rsp_body, rsp_header) {
                fail_callback(ret, param, rsp_body);
            })
 ...
 */
define.pack("./view",["webbase_css","disk_css","lib","common","$","./request","downloader","./tmpl","i18n"],function (require, exports, module) {
    ...
    var Event = function () {
        ...
        var click_file = function ($list) {
            ...
            $list.off('click', sel).on('click', sel, function () {
                ...
                //　判断点击文件类型，可预览文件则启动预览
                else if (FileObject.is_preview_doc(meta.name)) {
        ...
        var preview_image = function ($files, $file) {
            ...
            parent.seajs.use('image_preview', function (mod) {
                ...
                var index = $.inArray($file.get(0), $files);
                image_preview.start({
                    support_nav: true,
                    total: $files.length,
                    index: index,
        ...
        var preview_doc = function ($file) {
            ...
            var meta = get_meta($file);
            parent.seajs.use('doc_preview', function (mod) {
        ...
        var get_url = function ($file) {
            ...
            var url = urls.make_url('http://preview.cgi.weiyun.com:8080/weiyun_web_download.fcg', {
    ...
    var View = function () {
        ...
        var enter_dir = function (path, enter_flag, is_init_root) {
            ...
            // 拉取列表
            request.list(compress_instance.get_file(), paths.join('/'), function (ret, params, body) {
                is_loading = false;
                ...
                if (ret === 10408) {
                    return Loading.err_encryption(_(l_key,'该压缩包已经加密，无法预览')+
                        '<br>'+_(l_key,'请<a data-action="down" href="#">下载</a>后查看'));
                }
                if (ret === 10401) {
                    return Loading.err_encryption(_(l_key,'文件已损坏'));
                }
...
__p.push('    ');
    ...
    __p.push('    <div data-name="error" class="viewer-tips" style="">\r\n\
        <div class="ui-tips">\r\n\
            ...
            //hyytodo\r\n\
            <p class="ui-text">');
_p(_(l_key,'拉取数据时发生错误'));
__p.push('                ，');
_p(_(l_key,'请<a data-action="retry" href="#">重试</a>或直接<a data-action="down" href="#">下载</a>'));
__p.push('</p>\r\n\
        </div>\r\n\
    </div>');
