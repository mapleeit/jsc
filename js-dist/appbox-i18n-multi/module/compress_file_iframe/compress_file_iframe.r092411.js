...
define("club/weiyun/js-dist/appbox-i18n-multi/module/compress_file_iframe/compress_file_iframe.r092411",["lib","common","$","downloader","i18n"],function(require,exports,module){
 ...
 * @date 13-5-8
 */
define.pack("./CompressFileIframe",["lib","common","$","downloader","./tmpl","i18n"],function (require, exports, module) {
    var lib = require('lib'),
    ...
    CompressFileIframe.prototype = {
        ...
        loading: {
            ...
            process: function (size) {
                ...
                if (typeof size === 'number') {
                    ...
                    if (size >= 0) {
                        loading_text.text(_(l_key,'已加载') + Math.max(1, percent) + '%');  // 最小值1
                    } else {
                        loading_text.text(_(l_key,'正在加载中...'));
                    }
                }
                else {
        ...
        error_size_limit: function () {
            //hyytodo
            var file = this._file,
                $wording = $('<span>'+text.format('压缩包大小超过{0}M，暂不支持预览',[size_limit_M])+
                    '，'+text.format(_(l_key,'请直接{0}'),['<a href="#">'+_(l_key,'下载')+'</a>'])+'</span>');
