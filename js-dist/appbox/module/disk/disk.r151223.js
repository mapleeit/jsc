...
define("club/weiyun/js-dist/appbox/module/disk/disk.r151223",["lib","common","$","main","file_qrcode"],function(require,exports,module){
        ...
        load: function (node, from_cache, page, reset_ui, size) {
            ...
            if(page == 0) {
                if(node.is_album_backup_dir()) {
                    view_switch.temp_to_photo_view();
                } else {
...
//                disk_event.trigger('file_list_load', node);
            ...
            if (!load_done) {
                ...
                var load_ok_fn = function(body, virtual_list) {
                    ...
                    dirs_files = me._sort_data(dirs_files[0], dirs_files[1]);
...
//        },
            ...
            if (target_node) {
                ...
                if (cover && target_node.has_nodes()) {
                    var kid_nodes = target_node.get_kid_nodes(),
                        ...
                        name_conflicted = kid_nodes ? collections.grep(target_node.get_kid_nodes(), function (node) {
        ...
        _sort_data: function (dirs, files) {
            var sort_meta = this._get_local_sort_meta();

            //当字母序时，直接使用后台返回的数据，前台不再排序。因为中文文件名下会出现乱序
            if(sort_meta.field != 'name') {
        ...
        _get_sort_meta: function() {
            return {
                grid: [2, false], // 缩略图按时间倒序排列
                azlist: [1, false], // 按文件名顺序排序
                newestlist: [2, false] // 按时间倒序排序
            }[view_switch.get_cur_view()];  // 0-不排序(速度最快) 2-按修改时间排序；1-按名字排序,汉字基于拼音顺序(速度最慢)
        },
                ...
                case 'download_offline':
                    return {
                        ...
                        click: function (e) {
                            var files = file_list.get_selected_files();
                            if (files.length) {
                                file_list_ui_offline.download(files, e);
 ...
 * @date 13-3-5
 */
define.pack("./file_list.rename.ui",["lib","common","$","./tmpl","./file_list.ui_normal","./file_list.rename.rename"],function (require, exports, module) {
    var lib = require('lib'),
        common = require('common'),
...
//            this._fix_wk_layout();
        ...
        toggle_sidebar: function (visible) {
            ...
            main_ui.get_$ui_root().toggleClass('dbview-module', visible);
            this._fix_wk_layout();
        },
...
__p.push('    ');
    ...
    __p.push('    <div class="mod-dirbox">\r\n\
        <div class="dirbox-file">\r\n\
            <span class="fileimg">\r\n\
                <i data-id="icon" class="filetype icon-');
_p( first.get_type() );
__p.push('"></i>\r\n\
            </span>\r\n\
            <span class="filename">');
...
_p(is_tpmini ? 'list-tpmini': '');
__p.push('">');

            }
         if (!file.is_vir_dir()) { __p.push('                ');
 if (read_mode) { __p.push('                    <a data-file-check class="checkbox" tabindex="0" aria-label="');
_p(is_dir ? '目录：' + text_name + '，点击进入' : '文件：' + text_name);
__p.push('" href="#">&nbsp;</a>');
 } else { __p.push('                    <label data-file-check class="checkbox"></label>');
 } __p.push('            ');
 } __p.push('            <span data-action="enter" class="img"><i data-quick-drag data-ico class="filetype ');
_p( icon_cls );
__p.push('"></i>');
 if(is_tpmini) { __p.push('                <i class="tpmini-ico"></i>');
 } __p.push('            </span>\r\n\
...
__p.push('</span></em></p></span>');
 if(is_tpmini) { __p.push('                <span class="tpmini-state ');
_p(tpmini_fail ? 'tpmini-state-err' : 'tpmini-state-normal');
__p.push('">');
_p(tpmini_fail ? 'TP mini上传失败': 'TP mini加速上传中...');
__p.push('</span>');
 } __p.push('            <span data-action="enter" class="tool">');
 if (is_removable) { __p.push('                <a data-function="remove" title="删除" href="#" hidefocus="on" tabindex="0" ');
_p(click_tj.make_tj_str('ITEM_DELETE'));
__p.push('><i class="ico-del"></i></a>');
 } __p.push('            ');
 if (is_renamable) { __p.push('                <a data-function="rename" title="重命名" href="#" hidefocus="on" tabindex="0" ');
_p(click_tj.make_tj_str('ITEM_RENAME'));
__p.push('><i class="ico-rename"></i></a>');
 } __p.push('            ');
 if (is_movable) { __p.push('                <a data-function="move" title="移动" href="#" hidefocus="on" tabindex="-1"><i class="ico-move"></i></a>');
 } __p.push('            ');
 if (!file.is_tempcreate() && !is_broken && !file.is_vir_dir() && !is_tpmini) { __p.push('                <a data-function="share_enter" title="分享" tabindex="0" ');
_p(click_tj.make_tj_str('ITEM_SHARE'));
__p.push('  href="#" hidefocus="on"><i class="ico-share"></i></a>');
 } __p.push('            ');
 if (is_downable) { __p.push('                <a data-function="download" title="下载" href="#" hidefocus="on" tabindex="0" ');
_p(click_tj.make_tj_str('ITEM_DOWNLOAD'));
__p.push('><i class="ico-download"></i></a>');
 } __p.push('            </span>');
 if(!is_tpmini) { __p.push('                ');
 if (read_mode && !file.is_vir_dir()) { __p.push('                    ');
 if (!is_dir) { __p.push('                        <div data-action="enter" class="size" tabindex="0">大小：');
_p( file.get_readability_size(true) );
__p.push('</div>');
 } __p.push('                    <div data-action="enter" class="time" tabindex="0">时间：');
_p( scr_reader_mode.readable_time(create_time) );
__p.push('</div>');
 } else { __p.push('                    <span data-action="enter" class="size">');
...
__p.push('</span>');
 ...
 if (!is_dir && !is_broken) { __p.push('                    <span class="dimensional" data-action="enter">\r\n\
...
_p(cur_cls);
__p.push('" href="#" title="排序方式" tabindex="-1"><i class="ico"></i></a>\r\n\
    <a data-action="sidebar" class="dbview-btn ');
_p(data.is_sidebar_view ? 'dbview-focus' : '');
__p.push('" title="');
_p( data.is_sidebar_view ? '隐藏目录树' : '查看目录树');
__p.push('" href="#" tabindex="-1"></a>\r\n\
