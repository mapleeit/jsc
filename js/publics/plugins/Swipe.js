function Swipe(tBox,args) {
    ...
    function directionDetect(l,t){
        if(tPoint.mode == 'left-right'&&Math.abs(l)>tPoint.iniL){
            var rStr=l<0?'left':'right';
            return rStr;
        } else if(tPoint.mode == 'up-down' && Math.abs(t)<tPoint.iniT){
            var rStr=t<0?'up':'down';
            return rStr;
        }
        return false;
    }
    ...
    function endFun(e){
        var tapHold = (new Date() - tPoint.startTime) > 500 && !tPoint.mX; //判断长按
        if(typeof tPoint.eCallback=="function" && !tapHold){
...
if (typeof define === 'function') {
    define("club/weiyun/js/publics/plugins/Swipe",[],function (require, exports, module) {
        return Swipe;
    });
}