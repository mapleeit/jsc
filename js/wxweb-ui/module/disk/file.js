...
function format(files, type, pdir_key, ppdir_key) {
	...
	for(var i=0, len=files.length; i<len; i++) {
		temp = files[i];
		old = this.getStore(temp['dir_key'] || temp['file_id']);
		//生成文件对象，最简化数据以保证setData的传输数据大小
		file = {
			pdir_key: temp.pdir_key,
			ppdir_key: temp.ppdir_key,
			ext_info: {}
		};
		if(temp.dir_key) {
		...
		//父目录索引
		if(!temp.pdir_key && pdir_key) {
			file.pdir_key = pdir_key;
		}
		if(!temp.ppdir_key && ppdir_key) {
			file.ppdir_key = ppdir_key;
		}
		//数据类型，文件夹/文件
		file.type = temp.type || type;
		//文件后缀名
		file.ext = temp.ext || ext(temp.filename);
		//文件大小
		...
		//操作栏动画数据
		if(!temp.opAnimationData) {
			file.opAnimationData = old ? old.opAnimationData : {};
		} else {
			file.opAnimationData = temp.opAnimationData;
		}
		if(temp.opStatus === undefined) {
			file.opStatus =  old ? old.opStatus : false;
		} else {
			file.opStatus = temp.opStatus;
		}
...
function isAudio(type) {
	return type in EXT_AUDIO_TYPES;
}
...
function isPreviewVideo(type) {
	return type in EXT_PREVIEW_VIDEO_TYPES;
}
