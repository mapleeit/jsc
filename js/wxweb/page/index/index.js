...
var	navIcon = ['all', 'doc', 'img', 'video', 'audio'],
	...
	fileBlank = [
		{ img: '../../img/status/icon-noall.png', title: '该目录下没有文件', subTitle: '' },
...
Page({
	onLoad: function(option) {
		...
		query = {
			ppdir_key: decodeURIComponent((option || {}).ppdir_key || ''),
			pdir_key: decodeURIComponent((option || {}).pdir_key || ''),
			dir_key: decodeURIComponent((option || {}).dir_key || ''),
			dir_name: decodeURIComponent((option || {}).dir_name || '')
		};

		if(query.dir_key && query.dir_name) {
			current = query.dir_key;
			wx.setNavigationBarTitle({ title: query.dir_name });   //更新标题栏
		} else {
			//pv上报，只有首页需要，目录列表不上报
			global.mta.rptMain();
		}
	...
	onReady: function() {
		...
		function DiskUserInfoGetHandler() {
			pageInited = 2;
			//默认进入第一个tab
			me.loader().then(function() {
				if(current.toString().length === 1) {
					me.render({
						extend: { navDisplay: true }
					});
				} else {
					me.render();
				}
    ...
    handleData: function(res) {
        ...
        if (fetchDataType == 'dir') {
            data        = (res.dir_list || [])[0] || {};
	        ppdir_key   = query.pdir_key || global.userInfo.root_dir_key;
	        pdir_key    = query.dir_key || global.userInfo.main_dir_key;
	...
	render: function(opt) {
	    ...
	    listHeight = windowHeight - (current.toString().length === 32 ? 12 : me.isNav(1) ? 102 : 61);
	    var param = {
		    dialogDisplay: false,
	...
	catchOpTap: function(e) {
		...
		wx.showActionSheet({
			...
			success: function(res) {
				switch(res.tapIndex) {
					case 0:
						me.catchShareTap(key, data);
						break;
					case 1:
						me.catchRenameTap(key, data);
						break;
					case 2:
						me.catchRemoveTap(key, data);
						break;
	...
	bindItemTap: function(e) {
		...
		if(type === 'dir') {
			navigateUrl = '../index/index?ppdir_key=' + encodeURIComponent(data.ppdir_key || global.userInfo.root_dir_key)
				+ '&pdir_key=' + encodeURIComponent(data.pdir_key || global.userInfo.main_dir_key)
				+ '&dir_key=' + encodeURIComponent(key)
				+ '&dir_name=' + encodeURIComponent(name);
			wx.navigateTo({
				url: navigateUrl,
				fail: function(res) {
					md.log('[main page]: dir navigate: ' + res.errMsg);
					me.showDialog('受小程序策略限制，暂不支持打开' + dirLevelLimit + '级以上文件夹，请用微云APP查看');
					md.write();
				}
			});
			...
			if(video && !file.isPreviewVideo(data.ext)) {
				me.showDialog('小程序暂不支持' + data.ext + '视频文件，请用微云APP播放');
			...
			if(doc && size > documentLimitSize) {
				me.showDialog('小程序暂不支持大于10M的文档预览，请用微云APP查看');
				global.mta.eventStat('preview', {'doclimit': 'true'});
				return;
			}
			if(doc && data.ext === 'txt') {
				me.showDialog('小程序暂不支持txt文件预览，请用微云APP查看');
			...
			request.webapp({
				...
				if(fileInfo.https_download_url) {
					...
					if(doc) {
						...
						navigateUrl = '../video/index?video_src=' + encodeURIComponent(download_url)
							+ '&video_name=' + encodeURIComponent(name)
							+ '&video_cover=' + encodeURIComponent((data.ext_info || {}).https_url + '/0')
							+ '&long_time=' + data.long_time;
						wx.hideToast();
						wx.navigateTo({
							url: navigateUrl
						});
						md.log('[main page]: navigateTo video page: ' + download_url);
						global.mta.eventStat('preview', {'video': 'true'});
					}
				} else {
					wx.hideToast();
					me.showDialog('预览链接获取失败，请重试');
	...
	bindVideoError: function(e) {
		...
		this.showDialog('小程序不支持该视频格式，请用微云APP播放');
	},
	...
	catchShareTap: function(key, data) {
		var me = this;
		var type, pdir, name, dir_key, file_id;

		if(!data) {
			return;
		}

		var shareAdd = function(userInfo) {
			type = data.type;
			pdir = data.pdir_key;
			name = data.dir_name || data.filename;
			if(type === 'dir') {
		...
		global.Wns.getUserInfo().then(function(userInfo) {
			shareAdd(userInfo);
		}, function() {
			shareAdd();
		});
	},
	catchMoveTap: function(key, data) {
		var me = this;
	...
	catchRenameTap: function(key, data) {
		var name;
	...
	catchRemoveTap: function(key, data) {
		var me = this;
		var option = {};
	...
	data: {
		...
		fileBlankData: { img: '../../img/status/icon-noall.png', title: '该目录下没有文件', subTitle: '' },
		//dialog提示
		dialogDisplay: false,
		dialogText: '',
